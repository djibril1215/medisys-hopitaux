...
// Créer un transfert : récupère les données du patient depuis medisys-patients avant de les enregistrer
const createTransfert = async (req, res) => {
  ...
  try {
    // Récupération des données du patient depuis medisys-patients
    const patientResponse = await axios.get(
      `${process.env.PATIENTS_SERVICE_URL}/api/patients/${patient_id}`,
    ...
    if (error.response && error.response.status === 404) {
      return res.status(404).json({ message: 'Patient non trouvé dans le service patients.' });
    }
...
// Valider un transfert (l'hôpital destinataire accepte le patient)
const validerTransfert = async (req, res) => {
  const { id } = req.params;

  try {
    const result = await pool.query(
      `UPDATE transferts SET statut = 'accepte' WHERE id = $1 RETURNING *`,
      [id]
    );
    ...
    res.status(200).json({ message: 'Transfert accepté avec succès.', transfert: result.rows[0] });
  } catch (error) {
    console.error(error);
